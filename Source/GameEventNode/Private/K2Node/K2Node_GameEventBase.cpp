...
{
	return NSLOCTEXT("GameEventNode", "GameEventBase_Keywords", "event game system gameevent game event");
}
...
void UK2Node_GameEventBase::CreateEventIdentifierPins(const int32 Difference)
{
	GAME_SCOPED_TRACK_LOG_AUTO_BLUEPRINT_NAME()

	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();

	const int32 CurPinIndex = Pins.Num() - Difference;
	FCreatePinParams PinParams;
	PinParams.Index = FMath::Max(0, CurPinIndex);
	
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_String, FGameEventBasePinNames::EventStringPinName, PinParams);

	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, FGameplayTag::StaticStruct(), FGameEventBasePinNames::EventTagPinName, PinParams);

	UEdGraphPin* EventIdTypePin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Byte, StaticEnum<EEventIdType>(), FGameEventBasePinNames::EventIdTypePinName, PinParams);
